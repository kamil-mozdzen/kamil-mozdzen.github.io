...
/**
  * Klasa odpowiadająca strukturze ARMat typedef struct { double *m; int row; int clm; }ARMat;
  *
  */
NyARMat = Klass(
  {
    /**
      * Zwróć uwagę, że rozmiar tablicy i rozmiar macierzy nie muszą się koniecznie zgadzać. Nie używaj rozmiaru zwróconej tablicy jako rozmiaru macierzy!
      *
      */
  ...
  /**
    * Oblicza odwrotną macierz i zapisuje ją w this.
  ...
  ,matrixSelfInv : function()
  {
    ...
    var j, ip, nwork;
    var nos = this.__matrixSelfInv_nos;//zmienna robocza
    // double epsl;
    var p, pbuf, work;
    ...
    /*
 * Notatka nyatla: W przypadku, gdy adres IP nie jest ustalony i obliczenia są przeprowadzane, wstaw to. Nie jest pewne, czy można zainicjować zerem w pętli.
 */
    ip = 0;
// Zmiana kolejności w pętli jest niedozwolona
 ...
 * @param o_evec
 *     Proszę podać zmienną o dwóch elementach.
 * @param o_ev
 *     Proszę podać zmienną o dwóch elementach.
 * @param o_mean
 * @throws NyARException
...
NyARPca2d_MatrixPCA_O2 = ASKlass('NyARPca2d_MatrixPCA_O2', INyARPca2d,
{
  ...
  /**
   * static int QRM( ARMat *a, ARVec *dv )alternatywna funkcja
   *
   * @param a
   * @param dv
   * @throws NyARException
  ...
  PCA_QRM : function(o_matrix,dv)
  {
    ...
    // alternatywa // przekształć na macierz jednostkową
    mat00 = mat11 = 1;
    mat01 = mat10 = 0;
    // </this.vecTridiagonalize2d(i_mat, dv, ev);>
    // int j = 1;
    ...
    if (dv_y > t) {// if( dv->v[i] > t ) {
      ...
      // Brak zamiany miejscami wierszy
      o_matrix.m00 = mat00;
      o_matrix.m01 = mat01;
      o_matrix.m10 = mat10;
      o_matrix.m11 = mat11;
  ...
  ,PCA_PCA : function(i_v1,i_v2,i_number_of_data,o_matrix,o_ev,o_mean)
  {
    ...
    //Przetwarzanie PCA_CENTER i PCA_xt_by_x jednocześnie
  ...
  ,pca : function(i_v1,i_v2,i_number_of_point,o_evec,o_ev,o_mean)
  {
    ...
    // Zakaz zmiany kolejności w pętli for
...
INyARRgbPixelReader = ASKlass('INyARRgbPixelReader', {
  /**
   * "Zwraca pojedynczy piksel jako tablicę int
   *
   * @param i_x
   * @param i_y
   * @param o_rgb
  ...
  getPixel : function(i_x, i_y, o_rgb){},
  /**
   * Zwraca wartości wielu pikseli do i_rgb
   *
   * @param i_x
   * Tablica indeksów x
   * @param i_y
   * Tablica indeksów y
   * @param i_num
   * Liczba zwracanych wartości pikseli
   * @param i_rgb
   * Bufor zwracający wartości pikseli
   */
  getPixelSet : function(i_x, i_y, i_num, o_rgb){},
  /**
   * Ustawia 1 piksel
  ...
  setPixel : function(i_x, i_y, i_rgb){},
  /**
   * Ustawia wartości wielu pikseli z tablicy int
   * @param i_x
   * @param i_y
   * @param i_num
   * @param i_intrgb
...
NyARRgbPixelReader_INT1D_X8R8G8B8_32 = ASKlass('NyARRgbPixelReader_INT1D_X8R8G8B8_32', INyARRgbPixelReader,
{
  ...
  /**
   * Zmienia bufor, który jest używany jako odniesienie (referencja), na bufor przekazany jako i_ref_buffer.
Sprawdzanie wewnętrznych parametrów jest zależne od implementacji.
   * @throws NyARException
   */
  ,switchBuffer : function(i_ref_buffer)
  {
...
INyARDoubleMatrix = Klass({
  ...
  /**"Ustawia zawartość tablicy jako macierz.
Jest to wolne, więc staraj się tego unikać."
    * @param o_value
    */
  setValue : function(o_value){}, // double[]

  /**
    * "Zwraca zawartość macierzy jako tablicę.
Jest to wolne, więc staraj się tego unikać."
    * @param o_value
    */
...
NyARDoubleMatrix33 = Klass( INyARDoubleMatrix,
{
  ...
  /**
    * Ta funkcja zwraca wartość między 0 a PI
    * @param o_out
    */
  ...
  /**
    *Zastosuj macierz obrotu do transformacji współrzędnych.
    * @param i_angle
    * @param o_out
    */
...
/**
  * Tablica o zmiennym rozmiarze typu stosu.
Do tablicy wprowadza się rzeczywiste dane.

Uwagi:
Mimo prób obejścia ograniczeń Java Generics poprzez Vector.<*>, występuje wiele problemów, dlatego zmieniono to na Vector.<Object>.
Mogą wystąpić błędy w niektórych miejscach, jednak można próbować unikać ich, ustawiając opcje kompilatora itp. na strict = false.
Korekta podstawowa jest planowana w kolejnych wersjach.
...
NyARObjectStack = Klass(
{
  ...
  /**
    * Przygotuj dynamicznie alokowany bufor o maksymalnej liczbie `ARRAY_MAX`.
  ...
  initialize : function(i_length)
  {
    ...
    //Zresetuj ilość używanych elementów
    this._length = 0;
    return;
  }

  /**
    * "Miejsce określające, jakie rodzaje tablic (Vector) mają być przechowywane.
Proszę nadpisać tę funkcję."
    *
    */
  ...
  {
    //Alokuj w zależności od potrzeb
  ...
  /**
    * Zmniejsza widoczną liczbę elementów o 1 i zwraca ten obiekt.
Zawartość zwróconego obiektu jest ważna do następnego push.
  ...
  /**
    * Zmniejsza widoczną liczbę elementów o i_count sztuk.
    * @param i_count
    * @return
    */
  ...
  /**
 Zwraca pozorną liczbę elementów w tablicy
    * @return
    */
  ,getLength : function()
  {
  ...
  /**
    Resetuje pozorną liczbę elementów
    */
  ,clear : function()
  {
    this._length = 0;
...
NyARIntRectStack = Klass( //NyARObjectStack,
{
  ...
  initialize : function(i_length)
  {
   // Alokacja pamięci
    this._items = this.createArray(i_length);
    //Zresetuj liczbę używanych elementów
    this._length = 0;
    return;
  }
  ,createArray : function(i_length)
  ...
  */
, prePush: function() {
  // Alokuj, jeśli to konieczne
  if (this._length >= this._items.length) {
    return null;
  }
  // Zwiększ używaną przestrzeń o 1 i zwróć zarezerwowaną przestrzeń.
  var ret = this._items[this._length];
  this._length++;
  return ret;
}
...
/**
   * Pobiera bitmapę z obszaru zdefiniowanego przez prostokąt RECT: function(i_l,i_t,i_r,i_b) przy użyciu macierzy transformacji i_base_mat.
   * ----
   * Ta funkcja pobiera bitmapę z obszaru A płaszczyzny określonego przez prostokąt RECT(i_l,i_t,i_r,i_b), korzystając z macierzy transformacji i_base_mat.
   * Na przykład, dla prostokąta RECT(i_l,i_t,i_r,i_b) i wartości -40,0,0,-40.0, odpowiadającej 8 cm markerowi, funkcja pobierze obraz dolnej lewej części markera.
   *
   * Im dalej od markera, a także im bardziej odchylony od pionu markera, tym większe będą błędy.
  ...
  ,pickupImage2d : function(i_src_imege,i_l,i_t,i_r,i_b,i_base_mat)
  {
    ...
    // Przekształć 4 wierzchołki prostokąta na płaszczyźnie identycznej z markerem, przekształcając je za pomocą zmiany układu współrzędnych, a następnie obliczając wierzchołki na ekranie po transformacji perspektywicznej.
// [hX, hY, h] = [P][RT][x, y, z]
// Miejsce docelowe
